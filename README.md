...
2. npm will be installed from installing node
3. Run this command to execute django application non stop on server machine
...
- If we have to change the port number of our django application,
  then we can do that by the port no in args attribute of echosystem.config.json file.
- After done changes in echosystem.config.json file,
  stop the server using pm2 command like this:
  -
  ```
  pm2 start echosystem.config.json
  ```
  to again start the django application.
